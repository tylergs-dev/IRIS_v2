...
export function articleExtractionPrompt(links: EmailLink[], senderName: string): string {
  return [
    ...
    'links, "view in browser" links, links to the publication’s home page or section index,',
    'app store or subscription upsells, and one-sentence decks or blurbs that sit under a',
    'headline — those are descriptions of the article above, not separate articles.',
    '',
...
export function articleSummaryPrompt(title: string, text: string): string {
  return [
    ...
    'describing the article. If the page turned out to be a paywall or a login screen, say only',
    'that, and mention a helper can sign into that site in IRIS’s browser.',
    '',
    'Article text:',
    '---',
    text.slice(0, 16_000),
